...
/* Set display brightness using hardware PWM. */
#define ST77_BRIGHTNESS_LEVELS 32
#define ST77_MAX_BRIGHTNESS (ST77_BRIGHTNESS_LEVELS-1)
void st77xx_set_brightness(unsigned int level) {
    // Our perception of luminosity is highly non linear,
    // so we need gamma conversion of the input 32 bit levels
    // into 0-255 output.
    const uint8_t conv[32] = {
    0,1,2,3,4,5,7,9,12,15,18,22,27,32,38,44,51,58,
    67,76,86,96,108,120,134,148,163,180,197,216,235,255
    };
    unsigned int chan_level = conv[level&31];
    unsigned int slice_num = pwm_gpio_to_slice_num(st77_bl);
    unsigned int pwm_channel = pwm_gpio_to_channel(st77_bl);
    if (st77_bl != -1) {
        pwm_set_wrap(slice_num, 255);
        pwm_set_chan_level(slice_num, pwm_channel, chan_level);
        pwm_set_enabled(slice_num, level != 0);
    }
}
...
void st77xx_init(void) {
    ...
    if (st77_bl != -1) {
        gpio_set_function(st77_bl, GPIO_FUNC_PWM);
        st77xx_set_brightness(ST77_BRIGHTNESS_LEVELS);
    }
}
